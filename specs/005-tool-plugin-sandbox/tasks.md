...
- [ ] T001 创建目录骨架：tests/unit/tool/ 目录、tests/unit/tool/__init__.py、
      tests/unit/tool/fixtures/ 目录
- [ ] T002 [P] 创建测试用示例脚本 tests/unit/tool/fixtures/echo_args.py：
      从 stdin 读取 JSON 参数，原样打印到 stdout 后正常退出（exit 0）
- [ ] T003 [P] 创建测试用示例脚本 tests/unit/tool/fixtures/sleep_forever.py：
      不读取 stdin，sleep 一个远超测试超时配置的时长（用于触发超时）
- [ ] T004 [P] 创建测试用示例脚本 tests/unit/tool/fixtures/exit_nonzero.py：
      打印一段说明文字到 stderr 后以非零退出码（如 1）退出
- [ ] T005 [P] 创建测试用示例脚本 tests/unit/tool/fixtures/grow_memory.py：
...
- [ ] T006 实现配置与异常 src/kernel/tool/sandbox_models.py：frozen dataclass
...
- [ ] T007 [P] 实现沙箱运行器 src/kernel/tool/sandbox_runner.py：
...
- [ ] T008 [P] 创建测试公共设施 tests/unit/tool/conftest.py：示例脚本绝对路径
      fixture（拼接 sys.executable + fixtures/*.py）、SandboxLimits 短超时
      fixture（供超时类测试使用较小的 timeout_seconds 加速测试）
- [ ] T009 [P] SandboxLimits 与异常层级单元测试 tests/unit/tool/test_sandbox_models.py：
...
- [ ] T010 [US1] 实现工具注册中心 src/kernel/tool/registry.py：ToolRegistry
...
- [ ] T011 [P] [US1] 注册与查找单元测试 tests/unit/tool/test_registry.py：
...
- [ ] T012 [US2] 实现沙箱执行编排 src/kernel/tool/sandbox.py：SandboxedTool
...
- [ ] T017 [US3] 在 src/kernel/tool/sandbox.py 补全超时判定：用
      asyncio.wait_for 包裹子进程等待，超时后终止子进程（跨平台的
      进程终止方式）并抛 SandboxTimeoutError(timeout_seconds)（FR-006，
      所有平台生效）
- [ ] T018 [US3] 在 src/kernel/tool/sandbox.py 补全资源超限识别（仅
...
- [ ] T019 [P] [US3] 超时单元测试 tests/unit/tool/test_sandbox_timeout.py：
...
- [ ] T020 [P] [US3] 资源超限单元测试
      ...
      （不是静默跳过）
- [ ] T021 [P] [US3] 安全默认值单元测试 tests/unit/tool/test_sandbox_timeout.py
...
- [ ] T022 [P] 在 src/kernel/tool/sandbox.py 补全输出截断：读取 stdout 时
      按 SandboxLimits.max_output_bytes 截断，超出部分在返回内容末尾追加
      明确的截断标记文本（research.md R4，Edge Case）
- [ ] T023 [P] 实现操作遥测 src/kernel/tool/telemetry.py：tool_invoke_span
...
- [ ] T024 在 src/kernel/tool/sandbox.py 集成遥测：invoke() 全程经
...
- [ ] T025 [P] 输出截断与遥测单元测试 tests/unit/tool/test_sandbox_telemetry.py：
...
- [ ] T026 [P] 并发调用单元测试 tests/unit/tool/test_sandbox_concurrency.py：
...
- [ ] T027 [P] 创建演示脚本 examples/demo_tool_sandbox.py：注册可信工具与
      沙箱工具、演示重名拒绝、正常执行、超时、非零退出码四个场景并打印
      span（quickstart.md 第 2 节的预期输出）
- [ ] T028 按 quickstart.md 全流程验证：pytest 全绿（含 Windows 上资源
      超限测试的预期 SKIPPED，SC-001）→ demo 输出符合预期
      （SC-002/003/004）→ 计时确认 15 分钟内完成，修复发现的问题
- [ ] T029 更新 README.md roadmap：005 状态改为"已完成"
