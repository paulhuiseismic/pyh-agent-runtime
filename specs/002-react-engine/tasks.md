...
- [ ] T001 创建目录骨架：tests/unit/react/ 目录、tests/unit/react/__init__.py
...
- [ ] T002 实现数据结构 src/kernel/react/models.py：frozen dataclass Observation（success/content）、
...
- [ ] T003 [P] 实现思考阶段提示构造与解析 src/kernel/react/prompting.py：
...
- [ ] T004 [P] 实现步骤遥测 src/kernel/react/telemetry.py：react_step_span(step_index, action, tool_name) 上下文管理器，
...
- [ ] T005 [P] 创建测试公共设施 tests/unit/react/conftest.py：脚本化 stub provider 工厂
...
- [ ] T006 [P] models 与 prompting 单元测试 tests/unit/react/test_models.py, tests/unit/react/test_prompting.py：
...
- [ ] T007 [US1] 实现 ReactEngine 核心循环 src/kernel/react/engine.py：
...
- [ ] T008 [US1] 在 src/kernel/react/engine.py 中实现工具执行与失败捕获：
...
- [ ] T009 [US1] 包级导出 src/kernel/react/__init__.py：按 contracts/react-loop-api.md 导出 ReactEngine、
      Observation、StepBudgetExceededError；保留/替换 ReactLoop Protocol 定义（001 签名不变，移除 SingleShotReactLoop）
- [ ] T010 [P] [US1] 直接回答场景单元测试 tests/unit/react/test_engine_answer.py：
      stub provider 首步即返回 final_answer → engine 不发起工具调用直接返回（验收场景 US1-2）
- [ ] T011 [P] [US1] 工具调用场景单元测试 tests/unit/react/test_engine_answer.py（同文件追加）：
      stub provider 首步返回 call_tool、次步返回 final_answer，stub tool 返回固定观察 →
      engine 两步完成，返回值基于工具观察（验收场景 US1-1）
- [ ] T012 [P] [US1] 未注册工具容错单元测试 tests/unit/react/test_engine_answer.py（同文件追加）：
...
- [ ] T013 [US2] 在 src/kernel/react/engine.py 补全步数控制：run() 起始校验 max_steps 为正整数
...
- [ ] T014 [P] [US2] 步数耗尽单元测试 tests/unit/react/test_engine_step_budget.py：
      永远 call_tool 的 stub provider + max_steps=3 → 异常 steps_executed==3 且不超步（验收场景 US2-1，SC-002）
- [ ] T015 [P] [US2] max_steps 校验单元测试 tests/unit/react/test_engine_step_budget.py（同文件追加）：
      max_steps=0/-1/1.5 均在发起任何调用前被拒绝为 InvalidRequestError（未触发任何 provider 调用，验收场景 US2-2）
- [ ] T016 [P] [US2] 边界单元测试 tests/unit/react/test_engine_step_budget.py（同文件追加）：
...
- [ ] T017 [P] provider 异常边界单元测试 tests/unit/react/test_engine_provider_errors.py：
...
- [ ] T018 [US3] 在 src/kernel/react/engine.py 集成步骤遥测：每次循环迭代经 telemetry.react_step_span 包裹
...
- [ ] T019 [US3] 遥测单元测试 tests/unit/react/test_engine_telemetry.py：
...
- [ ] T020 [P] 创建演示脚本 examples/demo_react_stub.py：脚本化 stub provider + stub tool + ConsoleSpanExporter，
      依次演示直接回答/工具调用后回答/步数耗尽三种运行并打印 span（quickstart.md 第 2 节的预期输出）
- [ ] T021 按 quickstart.md 全流程验证：pytest tests/unit/react 全绿（SC-001）→ demo_react_stub 输出符合预期
      （SC-002/003/004）→ 计时确认 15 分钟内完成（SC-005），修复发现的问题
- [ ] T022 更新 README.md roadmap：002 状态改为"已完成"
